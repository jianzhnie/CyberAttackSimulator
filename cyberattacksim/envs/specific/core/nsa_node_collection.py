...
from cyberattacksim.envs.specific.core.nsa_node import Node


class NodeCollection:
    """Class representing a collection of nodes for the 18-node Ridley
    Environment."""

    def __init__(self, network: Tuple[np.array, dict],
    ...
    def get_connected_nodes(self, number: int) -> List[int]:
        """When given a node returns a list of all of the nodes connected to
        that node.
        ...
        if self.nodes[number].get_condition()[0]:
            ...
            return [
                i for i in range(0, len(self.nodes))
