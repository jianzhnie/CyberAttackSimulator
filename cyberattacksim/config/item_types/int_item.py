...
from cyberattacksim.config.core import (ConfigItem, ConfigItemValidation,
                                        ItemTypeProperties)
from cyberattacksim.exceptions import ConfigItemValidationError
...
class IntProperties(ItemTypeProperties):
    """The IntProperties class holds the properties relevant for defining and
    validating an int value."""
    ...
    def validate(self, val: int) -> ConfigItemValidation:
        ...
        :return: An instance of :class:`~cyberattacksim.config.item_types.ConfigItemValidation`.
        :raise: :class:`~cyberattacksim.exceptions.ConfigItemValidationError` when validation fails.
        """
        ...
        if val is not None and type(val) in self._allowed_types:
            try:
                ...
                elif (self.min_val is not None and not self.inclusive_min
                ...
                if self.max_val is not None and val > self.max_val:
                    ...
                    raise ConfigItemValidationError(msg)
                elif (self.max_val is not None and not self.inclusive_max
