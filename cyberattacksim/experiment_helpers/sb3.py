...
def init_env(env: str, experiment_id: str):
    """Use the Stable Baselines 3 Monitor wrappper to wrap an environment in
    order to enable monitoring.
    ...
    """
    wrapped_env = Monitor(gym.make(env),
                          filename=f'./logs/{experiment_id}',
                          allow_early_resets=True)

    return wrapped_env


def train_and_eval(agent_name: str, environment, training_timesteps: int,
                   n_eval_episodes: int):
    ...
    agent_list = [
        ...
        DQN(DQNMlp,
            environment,
            verbose=1,
            tensorboard_log='./logs/dqn-tensorboard'),
    ]
...
def print_policy_eval_metrics(agents: list,
                              evals: list,
                              raw_metrics=False) -> None:
