"""A collection of methods that a blue agent could use.

This includes multiple ways to defend a network by saving nodes or making them
...
class BlueActionSet:
    ...
    def reduce_node_vulnerability(self, node: Node) -> Tuple[str, Node]:
        ...
        if (new_vulnerability_score < self.network_interface.current_graph.
                node_vulnerability_lower_bound):
            new_vulnerability_score = (self.network_interface.current_graph.
                                       node_vulnerability_lower_bound)
    ...
    def make_safe_node(self, node: Node) -> Tuple[str, Node]:
        ...
        if (self.network_interface.game_mode.blue.action_set.make_node_safe.
                increases_vulnerability.value):
            # Modifies the vulnerability by a set amount (cannot increase it past the limit in the config file)
            change_amount = (self.network_interface.game_mode.blue.action_set.
                             make_node_safe.vulnerability_change.value)
        ...
        elif (self.network_interface.game_mode.blue.action_set.make_node_safe.
              gives_random_vulnerability.value):
