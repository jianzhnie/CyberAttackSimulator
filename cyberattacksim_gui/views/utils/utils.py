...
def db_manager(request: HttpRequest) -> JsonResponse:
    ...
    if request.method == 'POST':
        ...
        def create_network():
            network = NetworkManager.db.insert(network=Network(),
                                               name=item_name)
            return reverse(
                'network editor',
        ...
        def create_game_mode_from():
            game_mode = GameModeManager.db.get(
                request.POST.get('source_item_id'))
...
def update_game_mode(request: HttpRequest) -> JsonResponse:
    """Update the :attribute: `edited_forms` dictionary with the current state
    of the config and check for errors.

    Check the current contents of the :class:`ConfigForm <cyberattacksim_gui.forms.ConfigForm>` are valid
    using the criteria defined in the appropriate section of the :class:`~cyberattacksim.game_modes.game_mode.GameMode`
    ...
    if request.method == 'POST':
        ...
        if operation == 'save':
            ...
            section = game_mode_form.update_section(section_name=section_name,
                                                    form_id=form_id,
                                                    data=request.POST)
            if game_mode_form.game_mode.validation.passed:
                return JsonResponse({
                    'valid':
                    game_mode_form.game_mode.validation.passed
                })  # whether the complete game mode is valid
            else:
                return JsonResponse(
                    {'errors': json.dumps(section.get_form_errors())},
                    status=400)
