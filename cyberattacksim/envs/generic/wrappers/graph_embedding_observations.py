...
class FeatherGraphEmbedObservation(ObservationWrapper):
    """Gym Observation Space Wrapper that embeds the underlying environment
    graph using the Feather-G algorithm.

    This wrapper uses the Feather-G Whole Graph embedding algorithm to embed
    the underlying environment graph and then re-creates the observation space
    to include the embedding and all other observation space settings from the
    configuration file.
    """
    ...
    def __init__(self, env: GenericNetworkEnv, max_num_nodes: int = 100):
        ...
        self.new_ob_space_dim = env.calculate_observation_space_size(
            with_feather=True)
    ...
    def observation(self, observation: np.ndarray) -> np.ndarray:
        ...
        elif (self.env.network_interface.adj_matrix.all() !=
        ...
        if self.network_interface.game_mode.observation_space.node_connections.value:
            ...
            observation = np.concatenate(
                (self.latest_graph_embedding, extra_obs),
                axis=None,
                dtype=np.float32)
        else:
