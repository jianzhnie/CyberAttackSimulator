...
def get_summary_statistics(datadf: pd.DataFrame) -> dict:
    ...
    for inet in networks:
        net1 = datadf[datadf['nodes'] ==
                      inet].copy()  # copy of the starting dataframe
        ...
        for ialgo in unique_algo:
            ...
            for itype in unique_type:
                rwds = net1[(net1['algorithm'] == ialgo)
                            & (net1['type'] == itype)]['reward']
                lgts = net1[(net1['algorithm'] == ialgo)
                            & (net1['type'] == itype)]['lenght']

                sums[itype] = {
                    'rw_mean': np.mean(rwds),
