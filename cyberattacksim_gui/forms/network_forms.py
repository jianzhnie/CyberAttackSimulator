...
from cyberattacksim_gui.views.utils.helpers import NetworkManager
        ...
        types = {
            'Mesh': {
                ...
                'int': [{
                    ...
                    'size',
                    'description':
                    'The amount of connections between the nodes. (smaller values mean the connections or more sparse).',
                }],
            },
            'Star': {
                'float': [{
                    'label':
                    'star_group_connectivity',
                    'description':
                    'The amount of connections between the groups. (smaller values mean the connections or more sparse).',
                }],
                'int': [
                    {
                        'label':
                        'first_layer_size',
                        'description':
                        'The number of nodes to include in the first layer',
                    },
                    {
                        'label':
                        'star_group_size',
                        'description':
                        'The number of nodes to include in the groups',
                    },
                ],
            },
            'P2P': {
                'float': [
                    {
                        'label':
                        'inter_group_connectivity',
                        'description':
                        'The amount of connections between the groups. (smaller values mean the connections or more sparse).',
                    },
                    {
                        'label':
                        'P2P_group_connectivity',
                        'description':
                        'The amount of connections within the groups. (smaller values mean the connections or more sparse).',
                    },
                ],
                'int': [{
                    'label':
            ...
            'Ring': {
                'float': [{
                    ...
                    'break_probability',
                    'description':
                    'The likelihood of a break in the connections of the ring.',
                }],
                'int': [{
                    'label':
                    'ring_size',
                    'description':
                    'The number of nodes to include in the ring',
                }],
            },
        }
            ...
            for float_item in items['float']:
                field_elements[float_item['label']] = django_forms.FloatField(
                    widget=RangeInput(
                        attrs={
                            'class':
                            'form-control form-range slider-progress ' + name,
                            'step': '0.01',
                        }),
                    required=False,
...
class NetworkForm(django_forms.Form):
    """Django form representation of a
    ...
    random_entry_node_preference = django_forms.ChoiceField(
        ...
        choices=((t.name, t.name.replace('_', ' ').capitalize())
        ...
        **kwargs,
        ...
        if self.network:
            try:
                ...
                self.initial['set_random_entry_nodes'] = (
                    self.network.set_random_entry_nodes)
                self.initial['random_entry_node_preference'] = (
                    self.network.random_entry_node_preference.value)
                self.initial['num_of_random_entry_nodes'] = (
                    self.network.num_of_random_entry_nodes)

                self.initial['set_random_high_value_nodes'] = (
                    self.network.set_random_high_value_nodes)
                self.initial['random_high_value_node_preference'] = (
                    self.network.random_high_value_node_preference.value)
                self.initial['num_of_random_high_value_nodes'] = (
                    self.network.num_of_random_high_value_nodes)

                self.initial['set_random_vulnerabilities'] = (
                    self.network.set_random_vulnerabilities)
                self.initial['node_vulnerability_upper_bound'] = (
                    self.network.node_vulnerability_upper_bound)
                self.initial['node_vulnerability_lower_bound'] = (
    ...
    def update_doc_meta(self, data: QueryDict):
        ...
        if self.doc_metadata_form.is_valid():
            self.network.doc_metadata.update(
                ...
                if selector['min'] != selector['max']:
                    field_elements[f'{name}_min'] = django_forms.FloatField(
                        widget=RangeInput(attrs={
                            'class':
                            f'{name} multi-range-placeholder integer'
                        }),
                        required=False,
                        ...
                        label=name,
                        help_text=
                        f'Select networks based upon the number of {name} being within a given range.',
                    )
...
class NetworkFormManager:
    ...
    def get_or_create_form(cls, network_id: str) -> NetworkForm:
        ...
        :return: a dictionary representation of the sections of the :class: `~cyberattacksim.networks.network.Network`
        """
    ...
    def save_as_network(cls, network_form: NetworkForm) -> Network:
        ...
        :return: a valid instance of :class: `~cyberattacksim.networks.network.Network`
        """
    ...
    @classmethod
    def update_network_attributes(cls, network_id: str,
                                  data: QueryDict) -> NetworkForm:
        ...
        form = NetworkForm(
            network=network,
            data=data)  # create a new network form an update with the new data
        if form.is_valid() and settings.DYNAMIC_UPDATES:
            NetworkManager.db.update(
                form.network)  # update the network in the database
        cls.network_forms[network_id] = form

        return form

    @classmethod
    def update_network_elements(cls, network_id: str,
                                data: dict) -> NetworkForm:
