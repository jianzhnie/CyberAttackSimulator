...
@register.filter
def to_id(value: str):
    """Replaces spaces with dashes in string argument to form html formatted
    id.
...
def format_text(text: str):
    """Return the text string with '_' replaced by spaces and first letter
    capitalized.
...
def set_cypress_ref(field: Field) -> Field:
    ...
    if hasattr(field.form, 'name'):
        field.field.widget.attrs.update(
            {'data-cy': field.form.name + '_' + field.name})
    else:
