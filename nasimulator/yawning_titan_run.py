...
class YawningTitanRun:
    ...
    def _args_dict(self):
        return {
            ...
            self.collect_additional_per_ts_data,
    ...
    def setup(self, new: bool = True, ppo_zip_path: Optional[str] = None):
        ...
        if self.output_dir:
            ...
            self.output_dir = pathlib.Path(
                os.path.join(AGENTS_DIR, 'trained', str(datetime.now().date()),
                             f'{self.uuid}'))
    ...
    def evaluate(
            self) -> Union[tuple[float, float], tuple[List[float], List[int]]]:
        """Evaluates the trained agent.

        :return: Mean reward per episode, std of reward per episode.
        """
        if self.agent:
            return evaluate_policy(self.agent,
                                   self.env,
                                   n_eval_episodes=self.n_eval_episodes)
        else:
            self.logger.error(
            ...
            for root, dirs, files in os.walk(self.output_dir):
                for file in files:
                    if file != 'INVENTORY':
                        ...
                        dir_path = file_path.replace(str(self.output_dir),
                                                     '')[1:]
        ...
        with open(os.path.join(unzip_path, 'INVENTORY'),
            ...
            for line in inventory_file.readlines()[1:]:
                ...
                if os.path.isfile(target_file_path):
                    ...
                    # File doesn't exist
                    _LOGGER.debug(
                        "   Verification failed, file doesn't exist.")
                    return False
