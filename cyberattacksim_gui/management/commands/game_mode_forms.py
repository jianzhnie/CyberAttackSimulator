...
from cyberattacksim.config.core import ConfigGroup, ConfigItem
from cyberattacksim.config.item_types.bool_item import BoolItem
from cyberattacksim.config.item_types.float_item import FloatItem
from cyberattacksim.config.item_types.int_item import IntItem
from cyberattacksim.game_modes.game_mode import GameMode
from cyberattacksim_gui.forms import RangeInput, create_doc_meta_form
from cyberattacksim_gui.views.utils.helpers import GameModeManager, next_key


class ConfigForm(django_forms.Form):
    """Base class for represent cyberattacksim config classes as html forms.
    ...
    def update_and_check(self) -> bool:
        ...
        if self.config_class.validation.passed:
            ...
            for k, i in self.config_class.get_config_elements(
                    ConfigItem).items():
...
class GameModeSection:
    """A representation of a section of a :class:
    `~cyberattacksim.game_modes.game_mode.GameMode`.

    Each group within the section has its items converted into a django form
    element and is assigned an icon string representing a bootstrap icon.
    """

    def __init__(self,
                 section: ConfigGroup = None,
                 form_name: str = None,
                 icon: str = None) -> None:
    ...
    def create_form_from_group(self,
                               group: ConfigGroup,
                               form_name: str = '',
                               tier: int = 0):
        """Create a representation of a single :class:
        `~cyberattacksim.config.core.ConfigGroup` element as a django form.
            ...
            if isinstance(e, BoolItem):
                ...
                el = django_forms.FloatField(
                    widget=RangeInput(
                        attrs={
                            'class':
                            'inline form-control form-range slider-progress',
                            'step': '0.01',
                        }),
                    required=False,
                    help_text=e.doc,
                ...
                el = django_forms.IntegerField(
                    widget=widgets.NumberInput(
                        attrs={'class': 'inline form-control'}),
                    required=False,
                    help_text=e.doc,
                    label=name,
                )
        ...
        class ConfigFormSubsection(ConfigForm):
            ...
            def __init__(self, *args, **kwargs):
                super().__init__(form_name, tier, group, field_elements, *args,
                                 **kwargs)

        self.form_classes.append(ConfigFormSubsection)
        self.forms.append(
            ConfigFormSubsection(data=group.to_dict(values_only=True)))
    ...
    def get_form_errors(self) -> Dict[str, Dict[str, List[str]]]:
        """Create a formatted dictionary of the errors in each of the forms
        that constitute the game mode section.
        ...
        return {
            i: {
                ...
                'items': {
                    k: v.fail_reasons
                    for k, v in
                    form.config_class.validation.element_validation.items()
                },
            }
            for i, form in enumerate(self.forms)
            if not form.config_class.validation.group_passed or any(
                not item.validation.passed
                for item in form.config_class.get_config_elements(
                    ConfigItem).values())
        }
...
class GameModeForm:
    ...
    `~cyberattacksim.game_modes.game_mode.GameMode` as a editable form.
    """

    game_mode: GameMode

    def __post_init__(self) -> None:
        """Initialise the individual sections representing all descendent
        :class:`~cyberattacksim.config.core.ConfigGroup`'s."""
        self.sections: Dict[str, GameModeSection] = {
            'red':
            GameModeSection(section=self.game_mode.red,
                            form_name='red',
                            icon='bi-lightning'),
            'blue':
            GameModeSection(section=self.game_mode.blue,
                            form_name='blue',
                            icon='bi-shield'),
            'game_rules':
            GameModeSection(
            ...
            'rewards':
            GameModeSection(section=self.game_mode.rewards,
                            form_name='rewards',
                            icon='bi-star'),
            'on_reset':
            GameModeSection(
    ...
    def update_section(self, section_name: str, form_id: int,
                       data: dict) -> GameModeSection:
        """Update the values of a specific component form within a section of
        the overall game mode form.
    ...
    def update_doc_meta(self, data: QueryDict):
        ...
        if self.doc_metadata_form.is_valid():
            self.game_mode.doc_metadata.update(
                **self.doc_metadata_form.cleaned_data)
            if settings.DYNAMIC_UPDATES:
                GameModeManager.db.update(self.game_mode)
...
class GameModeFormManager:
    """Create and manage sets of forms for a given :class:
    `~cyberattacksim.game_modes.game_mode.GameMode`.
    ...
    def get_or_create_form(cls, game_mode_id) -> GameModeForm:
        """Get or create the config forms for the current
        :param:`game_mode_id`.
        ...
        :return: a dictionary representation of the sections of the :class: `~cyberattacksim.game_modes.game_mode.GameMode`
        """
    ...
    def save_as_game_mode(cls, game_mode_form: GameModeForm) -> GameMode:
        """Create a complete config yaml file from a dictionary of form
        sections.

        :param game_mode_forms: dictionary containing django form objects representing sections of the config.

        :return: a valid instance of :class: `~cyberattacksim.game_modes.game_mode.GameMode`
        """
        if GameModeManager.db.get(game_mode_form.game_mode.doc_metadata.uuid):
            # TODO add description to params
            if not GameModeManager.db.get(game_mode_form.game_mode.doc_metadata
                                          .uuid).doc_metadata.locked:
...
class GameModeSearchForm(django_forms.Form):
    """A Django form object to represent the filterable components of a :class:

    `~cyberattacksim.game_modes.game_mode.GameMode`.
    """

    def __init__(self, *args, **kwargs):
        """A Django form object to represent the filterable components of a
        :class: `~cyberattacksimmm.game_modes.game_mode.GameMode`."""
        field_elements = {}
        fields = {}
        searchable_items = []
                    ...
                    if selector['min'] != selector['max']:
                        ...
                        field_elements[f'{name}_min'] = django_forms.FloatField(
                            widget=RangeInput(
                                attrs={
                                    'class':
                                    f'{name} multi-range-placeholder {_type} hidden'
                                }),
                            required=False,
                            help_text=item.doc,
                        ...
                        field_elements[
                            f'{name}_max'] = django_forms.FloatField(
                        ...
                        field_elements[name] = django_forms.BooleanField(
                            widget=widgets.CheckboxInput(
                                attrs={
                                    ...
                                    'switch',
                                    'class':
                                    f'{name} form-check-input inline hidden',
                                }),
                            required=False,
                            help_text=item.doc,
                            label=name,
    ...
    @property
    def filters(self):
        """Generate a dictionary of ranges or values that a game mode must have
        to be a valid query result."""
        filters = {
            n: self.cleaned_data[n]
            for n in self.changed_data if n != 'elements'
        }
        cleaned_filters = {}
