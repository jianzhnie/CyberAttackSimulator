...
class FixedRedAgent:
    """The `FixedRegAgent` provides the red activity for the FourNodeEnv
    ...
    def __init__(self,
                 skill=None,
                 exploit_capability_dev=10,
                 initial_no_of_zero_days=1):
        self.skill = skill
    ...
    def do_red_action(
        ...
        if red_action == 1:
            ...
            if attack >= attack_sucess_threshold:
                ...
                logger.debug(
                    f'Red Team: {attack} on target {target + 1} - FAILED')
