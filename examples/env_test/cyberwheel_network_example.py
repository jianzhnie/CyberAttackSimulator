...
# Create router
network.add_router('core_router', )
...
network.add_firewall_to_subnet('user_subnet',
...
network.add_firewall_to_subnet('server_subnet',
                               name='allow DNS',
                               src='user_subnet',
                               dest='server_subnet',
                               port=53)
...
network.add_subnet('dmz_subnet',
                   router_name='core_router',
                   ip_range='192.168.2.0/24')

# Add hosts to user_subnet
network.add_host('user01', 'user_subnet', 'example')
network.add_route_to_host('user01', '192.168.2.0/24', '192.168.0.1')

network.add_host('user02', 'user_subnet', 'workstation')
network.add_firewall_to_host('user02', name='foo', src='server01', port=3389)

network.add_host('user03', 'user_subnet', 'workstation')

network.add_host('admin_workstation', 'user_subnet', 'workstation')
network.add_firewall_to_host('admin_workstation',
                             name='allow SSH',
                             src='all',
                             port=22)
...
for i in range(1, 4):
    network.add_host(f'server0{i}', 'server_subnet', 'web_server')

for i in range(1, 4):
    network.add_host(f'dmz0{i}', 'dmz_subnet', 'workstation')
network.add_firewall_to_host('dmz03',
                             name='foo',
                             src='all',
                             dest='dmz03',
                             port=443,
...
network.add_host('dmz_jump_box', 'dmz_subnet', 'workstation')
network.add_firewall_to_host('dmz_jump_box', name='allow SSH', port=22)

# Add interfaces
network.add_interface('user01', 'dmz01')
