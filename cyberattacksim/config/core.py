...
class ConfigBase(ABC):
    ...
    def get_config_elements(
        self,
        types: Optional[Union[ConfigItem, ConfigGroup,
                              List[Union[ConfigItem, ConfigGroup]]]] = None,
    ) -> Dict[str, Union[ConfigItem, ConfigGroup]]:
        """Get the attributes of the class that are either :class:
        `ConfigGroup` or :class:`ConfigItem`.
    ...
    def get_non_config_elements(self) -> Dict[str, Any]:
        """Get all attributes of the class that are not :class: `ConfigGroup`
        or :class: `ConfigItem`.
    ...
    def config_items(self):
        """Property to represent the :class:

        `~cyberattacksim.config.core.ConfigItem` children of the group.
        """
    ...
    def config_groups(self):
        ...
        `~cyberattacksim.config.core.ConfigGroup` children of the group.
        """
    ...
    def stringify(self):
        ...
        strings.extend([
            f'{name}={val}'
            for name, val in self.get_non_config_elements().items()
        ])
    ...
    def __hash__(self) -> int:
        ...
        element_hash = [
            v.stringify() for v in self.get_config_elements().values()
        ]
        element_hash.extend([
...
class ItemTypeProperties(ABC):
    """An Abstract Base Class that is inherited by config data type
    properties."""
    ...
    def validate(self, val) -> ConfigItemValidation:
        """Perform the base validation checks common to all `ConfigItem`
        elements.
        ...
        try:
            if val is not None and type(val) not in self._allowed_types:
                msg = (f'Value {val} is of type {type(val)}, should be ' +
...
class ConfigValidationBase(ConfigBase):
    ...
    def __init__(
        ...
        fail_exceptions: Optional[Union[
            List[Union[ConfigGroupValidationError, ConfigItemValidationError]],
            Union[ConfigGroupValidationError,
                  ConfigItemValidationError], ]] = None,
    ):
        ...
        if isinstance(fail_exceptions, list):
            self.fail_exceptions: List[
                Union[ConfigGroupValidationError,
                      ConfigItemValidationError]] = fail_exceptions
        elif isinstance(fail_reasons, str):
            self.fail_exceptions: List[Union[ConfigGroupValidationError,
                                             ConfigItemValidationError]] = [
                                                 fail_exceptions
                                             ]
        else:
    ...
    def add_validation(self, fail_reason: str,
                       exception: ConfigGroupValidationError):
        """Add a validation fail_reason, exception pair to their respective
        lists.
    ...
    def passed(self) -> bool:
        """Returns True if there are no :attribute: `fail_reasons` or
        :attribute: `fail_exceptions`.
...
class ConfigItemValidation(ConfigValidationBase):
    """Create :class:`ConfigItemValidation` from
    :class:`ConfigValidationBase`."""
...
class ConfigGroupValidation(ConfigValidationBase):
    """Used to return a validation result for a group of dependant config
    items, and the list of item validations.
    ...
    def add_element_validation(
        ...
        """Add a :class:`ConfigItemValidation` or
        :class:`ConfigGroupValidation` to the item validation dict.
    ...
    def passed(self) -> bool:
        """Returns True if there are no :attribute: `fail_reasons` or
        :attribute: `fail_exceptions` and the group passed.
    ...
    def element_validation(
        ...
        """The dict of element to :class:`ConfigItemValidation` and
        :class:`ConfigGroupValidation` validations.
    ...
    def elements_passed(self) -> bool:
        """Returns True if all items passed validation, otherwise returns
        False.
...
class ConfigItem:
    ...
    def __setattr__(self, __name: str, __value: Any) -> None:
        """Set an attribute of the :class: `ConfigItem` if the value is to be
        set, call the validation method.
    ...
    def stringify(self) -> Any:
        """This is here to allow stringify methods to be call on both :class:

        `ConfigItem` and :class: `ConfigGroup` classes.
        """
        return self.value


class ConfigGroup(ConfigBase, ABC):
    """The ConfigGroup class holds a ConfigItem's, doc, properties, and a
    ...
    def validate(
        self,
        raise_overall_exception: Optional[bool] = False
    ...
    def to_legacy_dict(
            self,
            flattened_dict: Dict[str, Any] = None) -> Dict[str, ConfigItem]:
        """Convert the group into a unitary depth dictionary of legacy config
        value (aliases) to :class: `ConfigItem`'s.
    ...
    def set_from_dict(
        ...
        **kwargs,
        ...
        """Set the values of all :class: `ConfigGroup` or :class:`ConfigItem`
        elements.
        ...
        if infer_legacy:
            legacy = (True if all(
                k in config_dict
                for k in ['RED', 'BLUE', 'OBSERVATION_SPACE']) else False)
        if legacy:
            ...
            for element_name, v in config_dict.items():
                ...
                if isinstance(v, dict):
                    self.set_from_dict(v,
                                       legacy=True,
                                       root=False,
                                       _legacy_lookup=_legacy_lookup)
            ...
            for element_name, v in config_dict.items():
                ...
                elif not isinstance(v, dict) and isinstance(
                        element, ConfigItem):
                    element.set_value(v)
                else:
