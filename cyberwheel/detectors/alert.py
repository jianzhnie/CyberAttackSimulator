...
class Alert:
    ...
    def __init__(
        ...
        """A class for holding information on actions made by a non-blue agent.
        (Maybe we'll do a green agent at some point?)
        ...
        if self.src_host is not None:
            self.src_ip = self.src_host.mac_address
        if self.dst_hosts is not None:
