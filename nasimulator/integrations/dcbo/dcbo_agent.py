...
class DCBOAgent(object):
    """An agent class that provides the supporting methods for a DCBO based
    learner."""
    ...
    def act(self, observation, reward, done):
        ...
        This function is not completely implemented and is named to support
        random actions within OpenAI Gym envs.
        """
        return self.action_space.sample()

    def reset(self):
        """Reset the Agent back to initial config by resetting the
        `isolated_nodes`."""
        self.isolated_nodes = {}
    ...
    def predict(self, observation, reward, done, env):
        ...
        chosen_action = random.choices(
            ...
            weights=[
                float(i) / sum(self.probabilities) for i in self.probabilities
            ],
            k=1,
        )[0]
        ...
        if chosen_action == 1:
            ...
            try:
                node = list(env.network_interface.
                            get_all_node_blue_view_compromised_states().values(
                            )).index(1)
