"""General object for calling and creating routines for plotting RL results and
analysis."""
...
def plot_training_performance(
    ...
    if isinstance(data, list):
        ...
        for i in range(len(data)):
            ...
            temp_label = (model_names[i] + ', Final value ' +
                          str(np.ceil(running_mean[-2])))
            plt.plot(
                timesteps,
                data[i]['Reward'],
                alpha=0.5,
...
def plot_evaluation_performance(
    ...
    plt.errorbar(
        ...
        yerr=sigma_nohyp,
        fmt='D',
        label=r'$\gamma$=0.99, $\mathrm{LR}=3\times10^{-4}$',
    )
    plt.errorbar(xmid,
                 disc_factor,
                 yerr=sigma_disc_facto,
                 fmt='o',
                 label=r'$\gamma$=0.75')
    plt.errorbar(xhigh,
                 lear_rate,
                 yerr=sigma_learn_rate,
                 fmt='*',
                 label=r'$\mathrm{LR}$=0.001')

    # add the random agent performances
    # specify the width
    xrandom = [
        np.linspace(-0.5, 8.5, 8),
        np.linspace(8.5, 17.5, 8),
        np.linspace(17.5, 26.5, 8),
    ]
...
def plot_multi_extension_comparison_size(
    network_size: list,
    plot_order: list,
    ...
    """Routine to create the massive extension comparison using the evaluations
    obtained by each algorithms in the same networks as the training.
    ...
    for iplot, isize in enumerate(network_size):
        ...
        for yvertical in plot_order:
            ...
            # add the random data
            random_means.append(
                data_random.query(f'mod == "{yvertical}"')['mean'])
            random_std.append(
                data_random.query(f'mod == "{yvertical}"')['stddev'])
...
def plot_deployment_performances(
    order: list,
    sizes: list,
    ...
    """Routine to create the deployment comparison plot using the scores
    obtained by each algorithms in the training and realistic networks.
    ...
    for iplot, isize in enumerate(sizes):
        ...
        if isize == '22':
            plt.errorbar(
                xlow,
                data_by_size['mean_train'],
                yerr=data_by_size['std_train'],
                fmt='D',
                label=r'Training',
            )
            plt.errorbar(
                xhigh,
                data_by_size['mean_deploy'],
                yerr=data_by_size['std_deploy'],
                fmt='X',
            )
            plt.errorbar(
                xpoints + 0.1,
                random_results['mean'],
                yerr=random_results['stddev'],
                fmt='P',
                color='darkgreen',
            )
            plt.ylim(-4500, -20)
