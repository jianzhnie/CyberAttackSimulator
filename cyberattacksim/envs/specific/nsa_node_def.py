"""A new node network that can be configured for multiple different
configurations.
...
from cyberattacksim.envs.specific.core.nsa_node_collection import \
    NodeCollection
...
class NodeEnv(gym.Env):
    """Class that creates a similar environments to that presented in Ridley 17
    (Ref above)."""
    ...
    def __init__(
        ...
        if self.network is None:
            self.network = list(
                default_18_node_network().to_adj_matrix_and_positions())

        self.state = NodeCollection(self.network,
                                    self.chance_to_spread_during_patch)
        # skill, action_set, action_probabilities, node_set
        self.RED = NSARed(
            1,
            [3, 4],
            [
                self.spread_vs_random_intrusion,
                1 - self.spread_vs_random_intrusion
            ],
            [str(i) for i in range(self.state.get_number_of_nodes())],
        )

        self.action_space = spaces.Discrete(self.state.get_number_of_nodes() *
                                            2 + 1)
        self.observation_space = spaces.Box(
            low=0,
            high=1,
            shape=(self.state.get_number_of_nodes() *
