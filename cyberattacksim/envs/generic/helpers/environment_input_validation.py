...
def check_type(data: dict, name: str, types: list):
    """Check data item_types contained within a dictionary is one of a list of
    item_types.
    ...
    if type(data[name]) not in types:
        raise ValueError("'" + name + "' needs to be of type: " +
                         ' or '.join(map(str, types)))
...
def check_within_range(
    ...
    """Check that an item belonging to a dictionary fits within a certain
    numerical range (either inclusive or not).
