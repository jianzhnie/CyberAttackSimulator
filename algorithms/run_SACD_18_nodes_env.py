...
parser.add_argument('--EnvIdex', type=str, default='default_18_node_network', help='The environment name. Defaults to default_18_node_network')
...
parser.add_argument('--Max_train_steps', type=int, default=1e4, help='Max training steps')
parser.add_argument('--save_interval', type=int, default=1e2, help='Model saving interval, in steps.')
parser.add_argument('--eval_interval', type=int, default=1e1, help='Model evaluating interval, in steps.')
...
def main():
	#Create Env
	env = create_env(env_id=opt.EnvIdex)
	eval_env = create_env(env_id=opt.EnvIdex)
	...
	print('Algorithm: SACD','  Env:',opt.EnvIdex,'  state_dim:',opt.state_dim,
	...
	if opt.write:
		...
		writepath = './algorithms/sarl/logs/SACD/SACD_{}'.format(opt.EnvIdex) + timenow
		if os.path.exists(writepath): shutil.rmtree(writepath)
		writer = SummaryWriter(log_dir=writepath)

	#Build model
	if not os.path.exists('./algorithms/sarl/model'): os.mkdir('./algorithms/sarl/model/SACD')
	agent = SACD_agent(**vars(opt))
	if opt.Loadmodel: agent.load(opt.ModelIdex, opt.EnvIdex)

	if opt.render:
		while True:
			score = evaluate_policy(env, agent, 1)
			print('EnvName:', opt.EnvIdex, 'seed:', opt.seed, 'score:', score)
	else:
		total_steps = 0
		while total_steps < opt.Max_train_steps:
			...
			while not done:
				...
				if total_steps % opt.eval_interval == 0:
					...
					print('EnvName:', opt.EnvIdex, 'seed:', opt.seed,
