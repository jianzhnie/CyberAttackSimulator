...
class GraphExplore(gym.Env):
    ...
    This environment emulates a network and enables an agent to select which
    node to visit, if it is not possible to move to the node the agent is
    denied the move.
    """
    ...
    def __init__(
        ...
        # the initial random_seed of the random network generated
        self.random_seed = random_seed
        # the number of game moves allowed
        self.GAME_MAX = game_max
        self.visualisation = visualisation

        # Example when using discrete actions:
        if graph_name == 'path_graph':
            self.G = nx.path_graph(n=self.NODES)
        elif graph_name == 'complete_graph':
            self.G = nx.complete_graph(n=self.NODES)
        elif graph_name == 'random_internet':
            self.G = nx.random_internet_as_graph(n=self.NODES,
        ...
        self.observation_space = gym.spaces.Box(low=0,
                                                high=1,
                                                shape=(self.NODES, 1, 1),
                                                dtype=np.uint8)
        # all can communicate, or none can
        self.reward_range = (-1 * (self.NODES * self.NODES),
    ...
    def step(self, action: int) -> Tuple[np.array, float, bool, dict]:
        ...
        print('GAME STEP {_step} of {_max}'.format(_step=1 + self.CURRENT_STEP,
                                                   _max=self.GAME_MAX))
    ...
    def _take_action(self, action: int):
        ...
        if action == self.NODES:
            ...
            print('Currently at node:{current} and want to move to: {future}'.
                  format(current=self.POS_BLUE, future=action))
            # is possible?
            if action in list(self.G.neighbors(
                    self.POS_BLUE)) or (action == self.POS_BLUE):
                # move
                ...
                try:
                    ...
                    print('Never visited node {future} before'.format(
                        future=action))
    ...
    def _calc_reward(self) -> float:
        ...
        The defined reward as the total number of nodes visited with some
        penalty to try and coax the the agent into moving.
        """
    ...
    def _next_observation(self) -> np.array:
        ...
        The observation space is just a list of nodes visited and therefore the
        agent is blind to the connectivity space
        """
    ...
    def render(self, mode: str = 'live', close: bool = False):
        ...
        if mode == 'file':
            ...
            if self.visualisation is None:
                self.visualisation = CustomEnvGraph(
