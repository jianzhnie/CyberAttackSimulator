...
class NSARed:
    ...
    def choose_action(self) -> str:
        ...
        """
        action = random.choices(self.action_set,
                                weights=self.action_probabilities)[0]

        return action

    def update_location(self, target: int,
                        red_current_node: int) -> Tuple[int, int]:
    ...
    def increment_day(self):
        ...
        If the day has reached a threshold for a zero day then add 1 to the
        number of available zero days.
        """
    ...
    def zd_attack(self, target: int, args: Tuple[List[List[float]], int, int,
                                                 bool]):
    ...
    def basic_attack(self, target: int, args: Tuple[List[List[float]], int,
                                                    int, bool]):
        ...
        if attack >= attack_success_threshold:
            ...
            logger.debug(
                f'Red Team: {attack} on target {target + 1} - SUCCESS')
            if able_to_move:
                self.update_location(target, red_current_node)
        else:
    ...
    def move(self, target: int, args: Tuple[List[List[float]], int, int,
                                            bool]):
