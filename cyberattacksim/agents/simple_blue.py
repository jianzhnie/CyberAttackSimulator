...
class SimpleBlue:
    """An interface capable of training a reinforcement learning agent in the
    ...
    def recover_machines(
        ...
        """Recover a compromised machine and resets its state to the initial
        state during environment creation.
    ...
    def do_blue_action(
        ...
        if 0 <= action <= self.n_machines - 1:
            ...
            self.recover_machines(action - self.n_machines, machine_states,
                                  initial_states)
