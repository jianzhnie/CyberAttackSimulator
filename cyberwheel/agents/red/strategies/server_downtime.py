...
class ServerDowntime(RedStrategy):
    ...
    current host if it is an unknown host or a server that has not been impacted
    yet. Once it finds an impacted server, it looks for another server to attack.
    If no unimpacted servers or unknown hosts are available, the strategy signifies
    failure by potentially assigning a high cost.
    """

    @classmethod
    def select_target(cls, agent_obj) -> Optional[Host]:
        """
        Selects the next target host based on the current state of the agent.
        ...
        if (
            ...
            or agent_obj.unimpacted_servers.check_membership(current_host_name)
        ):
