"""Provides an API for the ``network.json`` TinyDB file, and a Schema class
that defines the network DB fields."""
...
class NetworkSchema(YawningTitanDBSchema):
    ...
    SET_RANDOM_ENTRY_NODES: Final[YawningTitanQuery] = YawningTitanQuery(
    ).set_random_entry_nodes
    RANDOM_ENTRY_NODE_PREFERENCE: Final[YawningTitanQuery] = YawningTitanQuery(
    ).random_entry_node_preference
    NUM_OF_RANDOM_ENTRY_NODES: Final[YawningTitanQuery] = YawningTitanQuery(
    ).num_of_random_entry_nodes
    SET_RANDOM_HIGH_VALUE_NODES: Final[YawningTitanQuery] = YawningTitanQuery(
    ...
    NUM_OF_RANDOM_HIGH_VALUE_NODES: Final[
        YawningTitanQuery] = YawningTitanQuery().num_of_random_high_value_nodes
    SET_RANDOM_VULNERABILITIES: Final[YawningTitanQuery] = YawningTitanQuery(
    ).set_random_vulnerabilities
    NODE_VULNERABILITY_LOWER_BOUND: Final[
        YawningTitanQuery] = YawningTitanQuery().node_vulnerability_lower_bound
    NODE_VULNERABILITY_UPPER_BOUND: Final[
        YawningTitanQuery] = YawningTitanQuery().node_vulnerability_upper_bound


class NetworkDB:
    """The :class:`~yawning_titan.db.networks.NetworkDB` class extends
    :class:`~yawning_titan.db.YawningTitanDB`.
    ...
    def insert(
        ...
        """Insert a :class:`~yawning_titan.networks.network.Network` into the
        DB as ``.json``.
    ...
    def all(self) -> List[Network]:
        """Get all :class:`~yawning_titan.networks.network.Network` from the
        network DB.
    ...
    def search(self, query: YawningTitanQuery) -> List[Network]:
        """Searches the :class:`~yawning_titan.networks.network.Network` with a
    ...
    def update(
        ...
        """Update a :class:`~yawning_titan.networks.network.Network`. in the
        db.
        ...
        if doc:
            ...
            network.doc_metadata.updated_at = doc['_doc_metadata'][
                'updated_at']
    ...
    def upsert(
        ...
        """Upsert a :class:`~yawning_titan.networks.network.Network`. in the
        db.
    ...
    def remove(self, network: Network) -> Union[str, None]:
        """Remove a :class:`~yawning_titan.networks.network.Network`. from the
        db.
    ...
    def remove_by_cond(self, cond: QueryInstance) -> List[str]:
        """Remove :class:`~yawning_titan.networks.network.Network`. from the db
    ...
    def reset_default_networks_in_db(self, force=False):
        ...
        default_network_path = os.path.join(network_root, '_package_data',
                                            'network.json')
        ...
        for package_data_network in default_db.all():
            ...
            if not force and db_network:
                ...
                reset = True
            if reset:
                self._db.db.upsert(package_data_network,
                                   DocMetadataSchema.UUID == uuid)
                _LOGGER.info(f"Reset default network '{name}' in the "
...
def default_18_node_network() -> Network:
    """The standard 18-node network found in the Ridley, A. (2017) research
    paper.
