...
class EvalPrintout:
    ...
    def print_stats(self, game_stats_list: List[dict], total_games: int):
        """Print out the (averaged) stats from the last avg_every number of
    ...
    def calculate_metrics(
        ...
        avg_actions = {
            k: round(v / self.avg_every)
            for k, v in dict(cumulative_actions).items()
        }
        ...
        sorted_actions = sorted(combined_actions.items(),
