...
class Subnet(NetworkObject):
    def __init__(
        ...
        **kwargs,
        ...
        self.available_ips: List[Union[ipa.IPv4Address, ipa.IPv6Address]] = list(
        ...
        if dns_server:
            try:
                ...
                raise ValueError(f"Invalid DNS server IP: {dns_server}. Error: {e}")
        else:
    ...
    def __repr__(self) -> str:
        return (
            f"Subnet(name={self.name!r}, ip_network={self.ip_network!r}, "
    ...
    def __deepcopy__(self, memo: dict) -> "Subnet":
        ...
        """
        new_subnet = Subnet(self.name, self.ip_range, deepcopy(self.router, memo))
    ...
    def set_default_route(self) -> None:
        """
        Sets the default route for the subnet based on the router's interface IP address.
        """
        ...
        if ip_version == 4:
            self.default_route = Route(
                dest=ipa.ip_network("0.0.0.0/0"), via=default_route_via
            )
        elif ip_version == 6:
            self.default_route = Route(
                dest=ipa.ip_network("::/0"), via=default_route_via
            )

    def set_dns_server(self, ip: Union[ipa.IPv4Address, ipa.IPv6Address]) -> None:
        """
    ...
    def get_prefix_length(self) -> int:
        """
        Returns the prefix length of the subnet (e.g., 24 for 192.168.0.0/24).
    ...
    def get_unassigned_ips(self) -> List[Union[ipa.IPv4Address, ipa.IPv6Address]]:
        """
    ...
    def assign_dhcp_lease(self, host_obj) -> None:
        """
        Simulates a DHCP lease by assigning an IP address to a host and setting up its DNS and routes.
    ...
    def remove_connected_host(self, host) -> None:
        ...
        """
        self.connected_hosts = [h for h in self.connected_hosts if h.name != host.name]
    ...
    def get_nexthop_from_routes(self):
        """
        Placeholder for a method to determine the next hop from routing tables.
        """
