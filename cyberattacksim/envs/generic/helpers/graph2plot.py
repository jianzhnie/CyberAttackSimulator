...
def repeat_check(node: Dict, legend_list: List[Line2D]):
    """Checks if a node already exists by comparing the nodes colour and
    description with nodes already in the legend.
    ...
    for legend in legend_list:
        if (legend.get_markerfacecolor() == node['colour']
...
class CustomEnvGraph:
    ...
    def __init__(self, title: str = None):
        """Initialise the CustomEnvGraph that is used to visualise and render
        node environments.
        ...
        self.vis_ax = plt.subplot2grid(shape=(1, 1),
                                       loc=(0, 0),
                                       rowspan=1,
                                       colspan=1)
        plt.subplots_adjust(
    ...
    def render(
        ...
        for n in g.get_nodes():
            ...
            if n in made_safe_nodes:
                ...
                special_colour.append(
                    special_node_info['high_value_node']['colour'])
        ...
        plt.scatter(made_safe_x,
                    made_safe_y,
                    color='#4ef2e7',
                    s=324,
                    zorder=10)
        # plots any special nodes for the env
        plt.scatter(special_x,
                    special_y,
        ...
        info = (
            ...
            str(
                round(
                    statistics.mean([n.vulnerability_score
                                     for n in g.nodes]), 2)))
        ax = plt.gca()
        ax.legend(
