...
from cyberattacksim.exceptions import NetworkError
...
class Network(nx.Graph):
    ...
    def __init__(
        ...
        RandomEntryNodePreference = RandomEntryNodePreference.NONE,
        num_of_random_entry_nodes: int = 0,
        set_random_high_value_nodes: bool = False,
        random_high_value_node_preference:
        RandomHighValueNodePreference = RandomHighValueNodePreference.NONE,
        ...
        **kwargs,
        ...
        :param random_entry_node_preference: The random entry node placement
            ...
            cyberattacksim.networks.network.RandomEntryNodePreference.
        ...
        :param random_high_value_node_preference: The random high value node
            ...
            cyberattacksim.networks.network.RandomHighValueNodePreference.
        ...
        self.edges_map: Dict[str, Tuple[Any, Any]]
    ...
    def node_vulnerability_lower_bound(self) -> float:
        """The minimum value that a node within the networks vulnerability can
        take."""
    ...
    def num_possible_high_value_nodes(self) -> int:
        ...
        Number of possible high value nodes calculated by seeing how many nodes
        there are minus the entry nodes, then only having 15% of the nodes left
        over to be high value nodes.
        """
    ...
    def get_nodes(
        ...
        """Get all of the nodes from the network and apply a filter(s) to
        extract a specific subset of the nodes.
    ...
    def _generate_random_vulnerability(self) -> float:
        """Generate a single random vulnerability value from the lower and
        upper bounds.

        :returns: A single float representing a vulnerability.
        """
        return random.uniform(self.node_vulnerability_lower_bound,
    ...
    def _check_intersect(self, node: Node):
        ...
        if self.entry_nodes and self.high_value_nodes:
            uuids_intersect = [
                n.uuid
                for n in set(self.entry_nodes) & set(self.high_value_nodes)
            ]
            if uuids_intersect:
    ...
    def set_from_dict(
        ...
        """Set the values of existing network attributes from those contained
        in a dictionary.
    ...
    def add_nodes_from_list(self,
                            nodes_list: List[str],
                            remove_existing=False) -> None:
        """Add nodes to the graph with properties defined from a list.

        :param nodes_list: a list of node properties
        :param remove_existing: a boolean to indicate whether to remove existing nodes
        """
        nodes = [*self.nodes]
        if remove_existing:
    ...
    def add_edges_from_list(self,
                            edges_list: List[str],
                            remove_existing=False) -> None:
        """Add edges to the graph with properties defined from a list.

        :param edges_list: a list of edge properties
        ...
        for i, edge_tuple in enumerate(edges_list):
            if edge_tuple not in edge_tuples:
                edge_tuples.append(edge_tuple)
                node_u = self.get_node_from_name(edge_tuple[0])
                node_v = self.get_node_from_name(edge_tuple[1])
                self.add_edge(node_u, node_v)
                self.edges_map[i] = (node_u, node_v)

    def add_edges_from_dict(self,
    ...
    def reset_random_entry_nodes(self):
        ...
        If no entry nodes supplied then the first node in the network is chosen
        as the initial node.
        """
        try:
    ...
    def reset_random_high_value_nodes(self):
        """Sets up the high value nodes (HVNs) to be used by the training
        environment.
        ...
        if (possible_high_value_nodes is
                None):  # If there are none possible then try again
            self.reset_random_high_value_nodes()
    ...
    def to_dict(self, json_serializable: bool = False) -> Dict[str, Any]:
        ...
        d = {
            ...
            self.num_of_random_high_value_nodes,
        ...
        if json_serializable:
            ...
            d['edges'] = {
                k.uuid: {node.uuid: attrs
                         for node, attrs in v.items()}
                for k, v in d['edges'].items()
            }
            d['_doc_metadata'] = d['_doc_metadata'].to_dict()
        return d

    def to_adj_matrix_and_positions(
            self) -> Tuple[numpy.array, Dict[str, List[float]]]:
        """Represent the network by its adjacency matrix and a dictionary of
        node names to positions."""
        return nx.to_numpy_array(self), {
            n.name: n.node_position
            for n in self.nodes
        }
    ...
    def __repr__(self):
        net_str = (f'{self.__class__.__name__}('
