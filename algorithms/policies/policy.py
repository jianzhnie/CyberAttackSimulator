...
class OurRNN(nn.RNN):
    def __init__(self, *args, **kwargs):
        ...
        # self.iii = iii
        # self.ooo = ooo
        # self.m = torch.nn.RNN(iii, ooo, 4)

    def forward(self, x):
        output, _ = super(OurRNN, self).forward(x, None)
        # print("@@@@@@@1", output.shape)
        output = output.view(output.size(1), output.size(-1))
...
class CNNExtractor(nn.Module):
    ...
    def __init__(
        ...
        lll = [514, 35]
        for iii, curr_layer_dim in enumerate(pi_layers_dims):
            policy_net.append(Reshape1())
            policy_net.append(Conv1d(1, 1, 30))
            policy_net.append(nn.Linear(lll[iii], curr_layer_dim))
            policy_net.append(activation_fn())
            last_layer_dim_pi = curr_layer_dim
        # Iterate through the value layers and build the value net
        for iii, curr_layer_dim in enumerate(vf_layers_dims):
            value_net.append(Reshape1())
            value_net.append(Conv1d(1, 1, 30))
            value_net.append(nn.Linear(lll[iii], curr_layer_dim))
...
class RNNExtractor(nn.Module):
    ...
    def __init__(
        ...
        lll = [64, 64]
        for iii, curr_layer_dim in enumerate(pi_layers_dims):
            policy_net.append(Reshape2())
            policy_net.append(OurRNN(last_layer_dim_pi, curr_layer_dim))
            policy_net.append(nn.Linear(lll[iii], curr_layer_dim))
            # value_net.append(Squeeze())
            policy_net.append(activation_fn())
            last_layer_dim_pi = curr_layer_dim
        # Iterate through the value layers and build the value net
        for iii, curr_layer_dim in enumerate(vf_layers_dims):
            value_net.append(Reshape2())
            value_net.append(OurRNN(last_layer_dim_vf, curr_layer_dim))
            value_net.append(nn.Linear(lll[iii], curr_layer_dim))
            # value_net.append(Squeeze())
            value_net.append(activation_fn())
            last_layer_dim_vf = curr_layer_dim
...
class ACRNNPolicy(BasePolicy):
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        #     )
        # elif self.net_arch == "GNN":
        self.mlp_extractor = RNNExtractor(
            self.features_dim,
            net_arch=self.net_arch,
            activation_fn=self.activation_fn,
            device=self.device,
