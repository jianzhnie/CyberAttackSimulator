...
def check_element(el_dict: dict, n: int, include_unbounded: bool):
    ...
    """
    n = 0 if not n else n
    if (n < 0 or not isinstance(el_dict, dict)
...
class EntryNodeCompatibilityQuery(Query):
    ...
    :func:`tinydb.queries.Query.test`, These functions are specific to the
    compatibility of :class: `~cyberattacksim.game_modes.game_mode.GameMode` and
    :class: `~cyberattacksim.networks.network.Network` objects.
    """

    def works_with(self,
                   n: Union[int, Network],
                   include_unbounded: Optional[bool] = False) -> QueryInstance:
        """Tests the game mode can work with a network with the parameter with
        a value of ``n``.
        ...
        :param n: The target value of a field as an int or an instance of :class: `~cyberattacksim.networks.network.Network`.
        :param include_unbounded: Whether to include fields where part of the range is unbounded.
        :return: ``True`` if it does exist, otherwise ``False``.
        """
        if isinstance(n, Network):
            n = len(n.entry_nodes
                    ) if n.entry_nodes else n.num_of_random_entry_nodes
...
class HighValueNodeCompatibilityQuery(Query):
    ...
    def works_with(self,
        ...
        """
        if isinstance(n, Network):
            n = (len(n.high_value_nodes)
                 if n.high_value_nodes else n.num_of_random_high_value_nodes)
...
class NetworkCompatibilityQuery(Query):
    ...
    def compatible_with(
            self,
            n,
            include_unbounded: Optional[bool] = True) -> QueryInstance:
        ...
        :param n: The target value of a field as an instance of :class: `~cyberattacksim.networks.network.Network`.
        :param include_unbounded: Whether to include fields where part of the range is unbounded.
        :return: ``True`` if it does exist, otherwise ``False``.
        """

        def test_compatible_with(val: dict, n: Network, include_unbounded):
            if (not isinstance(val, dict) or not isinstance(
                    n, Network
            ) or not all(
                    k in val for k in
                ['entry_node_count', 'high_value_node_count', 'node_count'])):
                return False
            mapper = {
                'entry_node_count':
                len(n.entry_nodes)
            ...
            results = [
                check_element(e, mapper[k], include_unbounded)
