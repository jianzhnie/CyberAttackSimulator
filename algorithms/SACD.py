...
class SACD_agent():
    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            v_next = torch.sum(next_probs *
                               (min_next_q_all - self.alpha * next_log_probs),
                               dim=1,
                               keepdim=True)  # [b,1]
    ...
    def save(self, timestep, EnvName):
        torch.save(
            ...
            f'./algorithms/sarl/model/SACD/sacd_actor_{timestep}_{EnvName}.pth'
        )
        torch.save(
            self.q_critic.state_dict(),
            f'./algorithms/sarl/model/SACD/sacd_critic_{timestep}_{EnvName}.pth'
        )
